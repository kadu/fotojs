...
function createCronVideoJob(_config: any) {
  return new CronJob(_config.video_cron, async () => {
    ...
    shell.exec(`ffmpeg -framerate 30 -y -pattern_type glob -i "${dir}*.jpg" -s:v 800x600 -c:v libx264 -crf 17 -pix_fmt yuv420p -strict -2 -acodec aac ${dir}timelapse.mp4`).code;
